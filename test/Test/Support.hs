...
import HotSwap.Compiler.Protocol (CompileRequest (..))
...
buildRuntimeFixtures = do
    root <- createTemporaryDirectory "hot-swap-runtime-fixtures"
...
findProjectRoot = getCurrentDirectory >>= search
  where
    search directory = do
        markerExists <- doesFileExist (directory </> "hot-swap.cabal")
        if markerExists
            then pure directory
            else do
                let parent = takeDirectory directory
                if parent == directory
                    then fail "could not locate hot-swap.cabal"
                    else search parent
